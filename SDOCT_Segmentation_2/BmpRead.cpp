...
//关连文件：Bmp.h
//说明：1)要根据读取的图片先设定正确的BMP_WIDTH和BMP_WIDTH的值,BMP_WIDTH等在头文件中
//		定义
//		2)本程序只能读取灰度图
//		3)图像数组的定义为IMG_DATA[BMP_WIDTH][BMP_WIDTH]，为了方便逐列计数而不是逐行
...
{
	FILE *names;
	errno_t err = fopen_s(&names, "D:\\SDOCT\\BmpNames.txt", "r");
	if (err != 0)
	{
		cout << "文件打开失败" << endl;
		return 0;
...
//参数：IMAGEDATA IMG_DATA[BMP_WIDTH][BMP_WIDTH]
//返回值：成功1，失败0
//状态：完成
//修改记录：
//最后修改时间：2016.12.28
...
int main()
{
	...
	//均值滤波去噪
//	LaborLowNoise(IMG_DATA);
//	memset(BmpName, 0, LENGTH_NAME_BMP);
//	strcpy_s(BmpName, "AfterLabor.bmp");
//	cout << "保存到文件" << BmpName << endl;
//	SaveBmp(BmpName, IMG_DATA);
	...
	Find_Path(IMG_DATA);
	memset(BmpName, 0, LENGTH_NAME_BMP);
	strcpy_s(BmpName, "AfterFindPath.bmp");
	cout << "保存到文件" << BmpName << endl;
	SaveBmp(BmpName, IMG_DATA);

	//恢复图片的弯曲程度
	Bend(IMG_DATA,BrightPixel);
	memset(BmpName, 0, LENGTH_NAME_BMP);
	strcpy_s(BmpName, "Result.bmp");
	cout << "保存到文件" << BmpName << endl;
	SaveBmp(BmpName, IMG_DATA);
